...
UniValue getdgpinfo(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("maxblocksize", (uint64_t)ajwmDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (uint64_t)ajwmDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (uint64_t)ajwmDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ajwm balance of the wallet\n"
            "  \"stake\": xxxxxxx,           (numeric) the total ajwm stake balance of the wallet\n"
