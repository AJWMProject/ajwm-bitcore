...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//toniajwm edit
        return 0;
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress ajwmSenderAddress(request.params[2].get_str());
        if(ajwmSenderAddress.IsValid()){
            CKeyID keyid;
            ajwmSenderAddress.GetKeyID(keyid);
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // ajwm
