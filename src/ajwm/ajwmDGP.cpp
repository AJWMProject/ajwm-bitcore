...
bool AjwmDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule AjwmDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t AjwmDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AjwmDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AjwmDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AjwmDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AjwmDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AjwmDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AjwmDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AjwmDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void AjwmDGP::createParamsInstance(){
...
dev::Address AjwmDGP::getAddressForBlock(unsigned int blockHeight){
...
void AjwmDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AjwmDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AjwmDGP::parseStorageOneUint64(uint64_t& value){
...
void AjwmDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AjwmDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
