...
AjwmState::AjwmState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = AjwmState::openDB(_path + "/ajwmDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

AjwmState::AjwmState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute AjwmState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, AjwmTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createAjwmAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            ajwm::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> AjwmState::vins() const // temp
{
...
void AjwmState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* AjwmState::vin(dev::Address const& _a) const
{
    return const_cast<AjwmState*>(this)->vin(_a);
}

Vin* AjwmState::vin(dev::Address const& _addr)
{
...
// void AjwmState::commit(CommitBehaviour _commitBehaviour)
// {
...
void AjwmState::kill(dev::Address _addr)
{
...
void AjwmState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address AjwmState::createAjwmAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void AjwmState::deleteAccounts(std::set<dev::Address>& addrs){
...
void AjwmState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void AjwmState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
