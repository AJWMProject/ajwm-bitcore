...
void checkResult(bool isCreation, std::vector<AjwmTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    AjwmTxConverter converter(transaction, NULL);
    ExtractAjwmTX ajwmTx;
    BOOST_CHECK(converter.extractionAjwmTransactions(ajwmTx));
    std::vector<AjwmTransaction> result = ajwmTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(ajwmtxconverter_tests, TestingSetup)
