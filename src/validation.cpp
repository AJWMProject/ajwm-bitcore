...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // ajwm
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            for(AjwmTransaction ajwmTransaction : ajwmTransactions){
                ...
                if(ajwmTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s AJWM additional fees, %d delta bytes\n",
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(77000000000 + ((pindex->nHeight - 5000) * 5)) * COIN){
