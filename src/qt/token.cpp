...
{
    CBitcoinAddress ajwmAddress(strAjwmAddress);
    if(ajwmAddress.IsValid()){
        CKeyID keyid;
        ajwmAddress.GetKeyID(keyid);
...
bool ToAjwmAddress(const std::string& strHash160, std::string& strAjwmAddress)
{
    ...
    CKeyID keyid(key);
    CBitcoinAddress ajwmAddress;
    ajwmAddress.Set(keyid);
    if(ajwmAddress.IsValid()){
        strAjwmAddress = ajwmAddress.ToString();
        return true;
    }
    return false;
}
